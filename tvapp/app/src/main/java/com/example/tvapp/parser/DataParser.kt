...
class TskgParser {
    ...
    fun  parseMovieDetails(body: String): FormatedMovieDetails {
        val document: Document = Jsoup.parse(body)
        ...
        return FormatedMovieDetails(seasons = seasons, description = description)
    }

    fun parseCategoryPage(body: String, BASE_URL: String, title: String): FormatedMovies {
        val document: Document = Jsoup.parse(body)
        val section = document.select(".app-shows-container > .app-shows-item-full")

        val result = FormatedMovies(
            title = title,
